...
{
    /// <summary>
    /// A bounded per-session buffer that captures Verbose / Information / Progress / Warning
    ...
    internal sealed class StreamRecordRing
    {
                                ...
                                int parentActivityId, string recordType)
        {
            ...
            {
                Activity         = Truncate(activity),
                Status           = Truncate(status),
                PercentComplete  = percentComplete,
                CurrentOperation = Truncate(currentOperation),
                SecondsRemaining = secondsRemaining,
                ParentActivityId = parentActivityId,
                RecordType       = recordType
            };
            Add(StreamKinds.Progress, payload);
        }

        private void Add(string stream, object payload)
        {
            lock (_gate)
            {
                if (!RefillAndConsume())
        ...
        public StreamSnapshot ReadFrom(long fromSequence, int maxRecords)
        {
            lock (_gate)
            {
                ...
                return new StreamSnapshot(available, nextOffset, _totalDropped, _nextSequence);
            }
        }
        ...
        private static object MaybeTruncate(object payload)
        {
            if (payload is string s) return Truncate(s);
            return payload;
        }
    ...
    internal sealed class StreamSnapshot
    {
        ...
        public StreamSnapshot(IReadOnlyList<StreamRecord> records, long nextOffset,
                              long totalDropped, long producedSoFar)
        {
            Records       = records;
