...
The wait endpoint can also tee `Verbose`, `Information`, `Progress`,
`Warning`, and non-terminating `Error` records out of the running runspace
and stream them back during the long-poll. This page describes how to use
it from the SPE PowerShell module and what to know about the bounds, the
audit trail, and the policy interaction. For the underlying transport
mechanism (why records arrive within ~200 ms of being written, request
budget, fallback behaviour) see
[`remoting-long-poll.md`](remoting-long-poll.md).
...
| `-OnError`       | `Write-Error`, cmdlet `WriteError` (non-terminating only) | `Stream`, `Sequence`, `TimeUtc`, `Message`, `FullyQualifiedErrorId`, `CategoryInfo`, `PositionMessage`, `ScriptStackTrace` |

Records are unified-ordered: `Sequence` is monotonic across all five streams
combined, so an interleaved log view ("step 3 verbose, then 30% progress,
then step 4 verbose, then a non-terminating error") reflects emission order
on the server.

`-OnError` fires only for non-terminating errors. Terminating exceptions
(`throw`, unhandled cmdlet failures) bypass `Streams.Error`, surface via
`LastErrors` on the script session at post-Idle drain, and are returned by
`Wait-RemoteScriptSession` in its final batch like before. `TargetObject` is
intentionally not on the wire - it can be an arbitrary Sitecore Item or
other payload that would balloon size and risk leaking attached fields. If
you need the failed object, query it from the script and emit it via
`Write-Information` or include it in the script's return value.
...
| `Write-Error`, cmdlet `WriteError` (non-terminating)         | Error       | `-OnError`       |
| Terminating exceptions, `throw`                              | (not a stream) | none (read `LastErrors` on the session after wait) |
...
| Cap            | Value                  | Behavior on overflow                                                  | Counter           |
| -------------- | ---------------------- | --------------------------------------------------------------------- | ----------------- |
| Per-record     | 4 KB (characters)      | Truncated, suffixed with `[...truncated]`                             | `truncatedCount`  |
| Per-session    | ~1 MB approx           | Drop-oldest until the new record fits                                 | `droppedSize`     |
| Rate           | 100 records / second   | Excess writes dropped immediately                                     | `droppedRate`     |

The wait response carries each counter as a running total. The aggregate
field `droppedCount` is `droppedRate + droppedSize` and exists for clients
talking to pre-split servers. When the aggregate increases between two
calls, `Wait-RemoteScriptSession` emits a yellow-bar warning that names the
specific cap so the operator can pick the right fix:

```
WARNING: Stream buffer for job <id> dropped 5 record(s) (4 over rate cap, 1 over size cap).
```

(Old servers that don't split the counter still produce the legacy
"dropped N record(s) due to rate or size cap" form.)

If you see the rate-cap form in CI, your script is emitting faster than
100 records/sec - add throttling between emits. If you see the size-cap
form, individual records are large or the producer is far ahead of the
consumer - shrink message bodies or pull more often. The truncation
counter (`truncatedCount`) is informational; the truncated message still
arrives, just with a `[...truncated]` suffix.
...
`Wait-RemoteScriptSession` manages the cursor automatically. The default
flow needs no awareness of cursors: the cmdlet starts at the head of the
buffer and advances internally as records arrive.

For resume, the cmdlet exposes two opt-in parameters:

- `-OnCursor { param($c) ... }` - fires every time the server-issued cursor
  advances. Pipe `$c` to a file or any persistent store.
- `-Cursor <opaque>` - seeds the next wait with a previously captured
  cursor so records already observed are not replayed.

```powershell
# First wait, persist the cursor as it advances.
Wait-RemoteScriptSession -Session $s -Id $jobId `
    -OnVerbose { param($r) Write-Host $r.Message } `
    -OnCursor  { param($c) Set-Content -LiteralPath cursor.txt -Value $c }

# After Ctrl+C / network drop / shell restart - resume from the saved cursor.
Wait-RemoteScriptSession -Session $s -Id $jobId `
    -Cursor (Get-Content -LiteralPath cursor.txt -Raw) `
    -OnVerbose { param($r) Write-Host $r.Message }
```

Direct HTTP callers (see `remoting-raw-http.md`) pass `cursor=` (empty
string) on the first call and echo back the `cursor` field from the
response on subsequent calls.

The signing key is regenerated on every app-pool start, so cursors do not
survive a recycle: a recycled-key cursor returns HTTP 400 `invalid_cursor`
and `Wait-RemoteScriptSession` stops with a warning. Re-issue without
`-Cursor` to read from the head of the (post-recycle) buffer. Records
themselves do not survive a recycle either, so this also matches the
"NotFound" outcome you would get reading from any valid cursor against a
...
```
INFO  AUDIT [Remoting] action=progressStreamed session=<id> count=<N> errors=<E> dropRate=<R> dropSize=<S> truncated=<T> ip=<addr> rid=<rid>
```

When the drained batch contains at least one Error record, a second line
follows so operators can grep error volume directly:

```
INFO  AUDIT [Remoting] action=errorStreamed session=<id> count=<E> ip=<addr> rid=<rid>
```

The records themselves are never logged - only metadata. To audit content,
add `Sitecore.Diagnostics.Log` calls inside the script and read them via
the standard log inspection tools.
...
```json
{
  ...
  "streams": [
    ...
    { "stream": "error",    "sequence": 2, "timeUtc": "2026-04-29T13:00:00.002Z",
      "message": "Item not found", "fullyQualifiedErrorId": "ItemNotFound,Get-Item",
      "categoryInfo": "ObjectNotFound: (master:/sitecore/missing:String) [Get-Item]",
      "positionMessage": "At line:3 char:5", "scriptStackTrace": "at <ScriptBlock>, <No file>: line 3" }
  ],
  "cursor": "<opaque>",
  "droppedCount": 0,
  "droppedRate": 0,
...
`streams`, `cursor`, `droppedCount`, `droppedRate`, `droppedSize`, and
`truncatedCount` are present only when the caller passed a `cursor` query
param. Without that, the response shape is identical to pre-feature.
